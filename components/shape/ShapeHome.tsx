...
function ShapeHome({ Nome, Path }: Props) {
  ...
  return (
    <li className=" shadow-lg w-80 border-[1px] border-black bg-white rounded-lg hover:scale-105 ease-in-out duration-300">
      <h2 className="p-4 font-bold text-center">{Nome}</h2>
      <Image src={Path} className="p-1" alt='foto de um shape' width={300} height={300} />
      <div className="flex justify-center py-4 gap-2 cursor-pointer items-center ">
        ...
        <Link href={`products/${Nome}`} className="text-lg ">Comprar</Link>
      </div >
    </li>
  )
}
